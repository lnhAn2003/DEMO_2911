...
import { fetchChatMessage, deleteChatMessage } from '../../src/serverside/chatMessage';
...
const ChatRoomPage = () => {
  ...
  useEffect(() => {
    ...
    const handleDeleteMessage = (deletedId: number) => {
  ...
  const handleDeleteClick = () => {
    if (!selectedMessage) return;
      setDeleteTargetId(selectedMessage.id.toString());
      setShowDeleteConfirm(true);
  };

  // 11. Show delete modal
  const handleConfirmDelete = async () => {
    if (!deleteTargetId) return;
    const success = await deleteChatMessage(deleteTargetId);

    if (success) {
      alert('Message deleted successfully!');
      setMessages(prev => prev.filter(m => m.id.toString() !== deleteTargetId));
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white p-6 rounded shadow-lg w-1/3">
            ...
            <button
              ...
              className="mt-4 bg-red-500 text-white px-4 py-2 rounded hover:bg-red-700 ml-2"
            >
              Delete
            </button>
          </div>
